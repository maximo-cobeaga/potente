...
export function FormularioContacto({ oficina }) {
    const { register, handleSubmit, formState: { errors } } = useForm()
    async function submitFunction(e) {
        const { status } = await enviarContacto({ ...e, 'oficina': oficina })
    }
    const notify = () => {
        if (errors.nombre || errors.email || errors.telefono || errors.mensaje) {
    ...
    return (
        <form className='form-contacto' onSubmit={handleSubmit(submitFunction)}>
            <Toaster
                position="top-center"
                reverseOrder={false}
            />
